...
func (a *App) Requests(method string, url string, headers map[string]string, body string, options RequestOptions) HTTPResult {
	log.Printf("Requests: %v %v %v %v", method, url, headers, options)
	...
	if options.CancelId != "" {
		EventsOn(options.CancelId, func(data ...any) {
			log.Printf("Requests Canceled: %v %v", method, url)
			cancel()
		})
		defer EventsOff(options.CancelId)
	}
...
func (wt *WriteTracker) Write(p []byte) (n int, err error) {
	...
	if shouldEmit {
		EventsEmit(wt.ProgressChange, wt.Progress, wt.Total)
